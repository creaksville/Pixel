...
module.exports = {
    ...
    async execute(user, guild, client) {
        try {
            console.log(`User ${user?.user.id} has been banned in ${user?.guild.name}`);
            ...
            const banAddChannel = channelRecordRow[0]?.guildBanAdd;
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`:hammer: **${user?.user.username}** (${user?.user.id}) has been banned\n**Reason: ${reason}**.`)

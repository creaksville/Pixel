...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option
        .setName('target')
        .setDescription('Select where to add balance')
        .setRequired(true)
        .addChoices(
  ...
  async execute(interaction, client) {
    try {
      ...
      if (economyRows.length === 0) {
        ...
        await interaction.followUp(`Added ${amount} points to your ${target}. New balances:\nWallet: ${userEntry.wallet} points\nBank: ${userEntry.bank} points`);
      }

      connection.release();
    } catch (error) {
      console.error(error);
      await interaction.followUp('An error occurred while adding balance.');
    }
  },
};
